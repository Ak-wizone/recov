...
export const insertMasterCustomerSchema = createInsertSchema(masterCustomers).pick({
  ...
  creditLimit: z.string().min(1, "Credit limit is required").transform(val => val === "" ? null : val),
  openingBalance: z.string().optional().transform(val => !val || val === "" ? null : val),
  interestApplicableFrom: z.string().optional(),
  interestRate: z.string().min(1, "Interest rate is required").transform(val => val === "" ? null : val),
...
export const insertMasterCustomerSchemaFlexible = createInsertSchema(masterCustomers).pick({
  ...
  creditLimit: z.string().optional().transform(val => !val || val === "" ? null : val),
  openingBalance: z.string().optional().transform(val => !val || val === "" ? null : val),
  interestApplicableFrom: z.string().optional(),
  interestRate: z.string().optional().transform(val => !val || val === "" ? null : val),
  salesPerson: z.string().optional(),
  isActive: z.enum(["Active", "Inactive"]).default("Active"),
});
...
export const invoices = pgTable("invoices", {
  ...
  finalGpPercentage: decimal("final_gp_percentage", { precision: 8, scale: 2 }),
...
export const insertCommunicationScheduleSchema = createInsertSchema(communicationSchedules).pick({
  ...
  daysOffset: z.number().min(0).max(365).optional(),
