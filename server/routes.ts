...
import { AssistantOrchestrator } from "./services/assistantOrchestrator";
...
export async function registerRoutes(app: Express): Promise<Server> {
  // Initialize Assistant Orchestrator (OpenAI key loaded from environment)
...
${profile?.legalName || 'Company'}`;
  ...
  /**
   * Assistant Access Auth Helper - allows tenant users only
   * Platform admins cannot use assistant features
   */
  function ensureAssistantAccess(req: any, res: any, next: any) {
    const sessionUser = (req.session as any).user;
    ...
    // Assistant is for tenant users only - block platform admins
    if (!sessionUser.tenantId) {
      return res.status(403).json({ message: "Assistant features are available for tenant users only" });
    }
  ...
  app.delete("/api/assistant/history", ensureAssistantAccess, async (req, res) => {
    try {
  ...
  app.get("/api/assistant/history", ensureAssistantAccess, async (req, res) => {
    try {
      const userId = (req.session as any).user?.id;
      if (!userId) {
        return res.status(401).json({ message: "Unauthorized" });
  ...
  app.post("/api/assistant/command", ensureAssistantAccess, async (req, res) => {
    ...
    try {
      ...
      const { message, isVoice, context } = req.body;
      ...
      // Convert to orchestrator format - create alternating user/assistant turns
      const history: Array<{ speaker: "user" | "assistant"; message: string; timestamp: Date }> = [];
      conversationHistory.reverse().forEach(h => {
        // Each record has both user message and assistant response
        history.push({
          speaker: "user",
        ...
        history.push({
          speaker: "assistant",
          message: h.assistantResponse,
          timestamp: h.createdAt!,
        });
      });

      // Process with orchestrator
      const orchestratorResponse = await assistantOrchestrator.process({
        message,
        tenantId: req.tenantId!,
        userId,
      ...
      // Store in chat history (one record with both user message and assistant response)
      await db.insert(assistantChatHistory).values({
        tenantId: req.tenantId!,
        userId,
        userMessage: message,
        assistantResponse: orchestratorResponse.text,
        commandType: orchestratorResponse.type,
      ...
      await db.insert(assistantAnalytics).values({
        ...
        userId,
        command: message,
        normalizedCommand: message.toLowerCase().trim(),
        commandType: orchestratorResponse.type === "quick_command" ? "action" : "query",
        isSuccess: true,
        responseTime,
        isVoiceInput: isVoice || false,
      ...
      res.json({
        response: orchestratorResponse.text,
        type: orchestratorResponse.type,
        confidence: orchestratorResponse.confidence,
        command: orchestratorResponse.command,
        requiresAction: orchestratorResponse.requiresAction,
        actionType: orchestratorResponse.actionType,
        actionPayload: orchestratorResponse.actionPayload,
      });
    } catch (error: any) {
      console.error("[Assistant] Command processing error:", error);
  ...
  app.get("/api/assistant/settings", ensureAssistantAccess, async (req, res) => {
    try {
      const userId = (req.session as any).user?.id;
      if (!userId) {
        return res.status(401).json({ message: "Unauthorized" });
  ...
  app.put("/api/assistant/settings", ensureAssistantAccess, async (req, res) => {
    try {
      const userId = (req.session as any).user?.id;
      if (!userId) {
        return res.status(401).json({ message: "Unauthorized" });
  ...
  app.get("/api/assistant/analytics", ensureAssistantAccess, async (req, res) => {
    try {
      const userId = (req.session as any).user?.id;
      if (!userId) {
        return res.status(401).json({ message: "Unauthorized" });
  ...
  app.post("/api/telegram/generate-link-code", async (req, res) => {
    try {
      ...
      // First, mark all existing unused codes for this user as used/expired
      // This ensures only one active code exists at a time
      await db.update(telegramLinkingCodes)
        .set({ isUsed: true })
        .where(
          and(
            eq(telegramLinkingCodes.tenantId, req.tenantId),
            eq(telegramLinkingCodes.generatedByUserId, req.userId!),
