...
<body>
  ...
  // Tenant - Transcribe Audio with OpenAI Whisper
  app.post("/api/whisper/transcribe", upload.single("audio"), async (req, res) => {
    try {
      ...
      if (!req.file) {
        return res.status(400).json({ code: "NO_FILE", message: "No audio file uploaded" });
      }
      
      // Validate audio format (OpenAI Whisper supports: mp3, mp4, mpeg, mpga, m4a, wav, webm)
      ...
      // File size limit (25MB for OpenAI Whisper)
      ...
      // Get feature type from request body (default: voice_command)
      const feature = req.body.feature || "voice_command";
      const language = req.body.language || "auto"; // auto, en, hi, etc.
      ...
      if (!config) {
        return res.status(503).json({ 
          ...
          message: "Whisper service is not configured. Please contact platform administrator." 
        });
      }
      ...
      if (!config.enabled) {
        return res.status(503).json({ 
          ...
          message: "Whisper service is temporarily disabled" 
        });
      }
      ...
      try {
        openaiApiKey = decryptApiKey(config.encryptedApiKey);
      } catch (error: any) {
        console.error("[Whisper] Failed to decrypt API key:", error);
        return res.status(500).json({ 
          code: "KEY_DECRYPTION_ERROR", 
          message: "Failed to decrypt OpenAI API key. Please contact support." 
        });
      }
      ...
      let credits = await storage.getWhisperCredits(tenantId);
      if (!credits) {
        // Auto-initialize credits for first-time use
        credits = await storage.initializeWhisperCredits(tenantId);
        if (!credits) {
          return res.status(500).json({ 
            code: "CREDITS_INIT_ERROR", 
            message: "Failed to initialize credits" 
          });
        }
      }
      
      // Estimate audio duration (rough estimate: file size / bitrate)
      // For safety, we'll do a pre-check before transcription
      const estimatedMinutes = Math.ceil(req.file.size / (128 * 1024 / 8) / 60); // Assuming 128kbps
      const availableMinutes = (credits.planMinutes || 0) + (credits.addonMinutes || 0) - (credits.usedMinutes || 0);
      
      if (availableMinutes < estimatedMinutes) {
        return res.status(402).json({ 
          code: "INSUFFICIENT_CREDITS", 
          message: `Insufficient credits. You have ${availableMinutes.toFixed(1)} minutes remaining, but this file may require ${estimatedMinutes} minutes.`,
          creditsRemaining: availableMinutes
        });
      }
      ...
      try {
        ...
        // OpenAI SDK accepts Buffer with name property in Node.js
        const audioFile = Object.assign(req.file.buffer, {
          name: req.file.originalname || "audio.wav"
        });
        ...
        // Get actual duration from OpenAI response (if available)
        actualDurationSeconds = (transcription as any).duration || estimatedMinutes * 60;
        
        console.log(`[Whisper] Transcription completed in ${processingTime}s for ${actualDurationSeconds}s audio`);
      } catch (error: any) {
        ...
        if (error.status === 401) {
          return res.status(502).json({ 
            ...
            message: "Invalid OpenAI API key. Please contact platform administrator." 
          });
        }
      ...
      // Calculate cost and minutes used
      const actualMinutes = actualDurationSeconds / 60;
      const costPerMinute = 0.006; // OpenAI Whisper pricing: $0.006 per minute
      const cost = actualMinutes * costPerMinute;
      
      // Deduct credits transactionally
      try {
        const deducted = await storage.deductWhisperCredits(tenantId, actualMinutes, {
          feature,
          audioLengthSeconds: actualDurationSeconds,
          cost,
          transcript: transcription.text?.substring(0, 500) || "", // Save first 500 chars
          language: (transcription as any).language || language
        });
        
        if (!deducted) {
          // This shouldn't happen since we checked balance, but handle edge case
          return res.status(402).json({ 
            code: "INSUFFICIENT_CREDITS", 
            message: "Insufficient credits to complete transcription",
            creditsRemaining: availableMinutes
          });
        }
      } catch (error: any) {
        console.error("[Whisper] Failed to deduct credits:", error);
        return res.status(500).json({ 
          code: "CREDIT_DEDUCTION_ERROR", 
          message: "Transcription succeeded but failed to deduct credits. Please contact support." 
        });
      }
      ...
      res.json({
        ...
        metadata: {
          ...
          durationMinutes: actualMinutes.toFixed(2),
          language: (transcription as any).language || language,
          cost: cost.toFixed(4),
          creditsUsed: actualMinutes.toFixed(2),
