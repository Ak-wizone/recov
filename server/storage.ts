...
export interface IStorage {
  ...
  // Instant Recovery Request operations
  getRecoveryRequests(tenantId: string): Promise<RecoveryRequest[]>;
  getRecoveryRequest(tenantId: string, id: string): Promise<RecoveryRequest | undefined>;
  createRecoveryRequest(tenantId: string, userId: string, request: InsertRecoveryRequest): Promise<RecoveryRequest>;
  updateRecoveryRequest(tenantId: string, id: string, request: Partial<InsertRecoveryRequest>): Promise<RecoveryRequest | undefined>;
  deleteRecoveryRequest(tenantId: string, id: string): Promise<boolean>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Instant Recovery Request operations
  async getRecoveryRequests(tenantId: string): Promise<RecoveryRequest[]> {
    return await db.select().from(recoveryRequests).where(eq(recoveryRequests.tenantId, tenantId)).orderBy(desc(recoveryRequests.createdAt));
  }

  async getRecoveryRequest(tenantId: string, id: string): Promise<RecoveryRequest | undefined> {
    const [request] = await db.select().from(recoveryRequests).where(and(eq(recoveryRequests.tenantId, tenantId), eq(recoveryRequests.id, id)));
    return request;
  }

  async createRecoveryRequest(tenantId: string, userId: string, request: InsertRecoveryRequest): Promise<RecoveryRequest> {
    const [newRequest] = await db.insert(recoveryRequests).values({ ...request, tenantId, createdBy: userId }).returning();
    return newRequest;
  }

  async updateRecoveryRequest(tenantId: string, id: string, request: Partial<InsertRecoveryRequest>): Promise<RecoveryRequest | undefined> {
    const [updated] = await db
      .update(recoveryRequests)
      .set({ ...request, updatedAt: new Date() })
      .where(and(eq(recoveryRequests.tenantId, tenantId), eq(recoveryRequests.id, id)))
      .returning();
    return updated;
  }

  async deleteRecoveryRequest(tenantId: string, id: string): Promise<boolean> {
    const result = await db.delete(recoveryRequests).where(and(eq(recoveryRequests.tenantId, tenantId), eq(recoveryRequests.id, id))).returning();
    return result.length > 0;
  }
