...
// Tenant Registration Import
export interface ImportTenantRegistrationRow {
  businessName?: string;
  email?: string;
  ...
  industryType?: string;
  planType?: string;
  existingAccountingSoftware?: string;
  paymentMethod?: string;
}

export async function parseTenantRegistrationsFile(file: File): Promise<ImportTenantRegistrationRow[]> {
  return new Promise((resolve, reject) => {
    const reader = new FileReader();
    ...
    reader.onload = (e) => {
      try {
        ...
        const rows: ImportTenantRegistrationRow[] = jsonData.map((row: any) => ({
          businessName: String(row["Business Name"] || row.businessName || row.BusinessName || "").trim(),
          email: String(row["Email"] || row.email || row.EMAIL || "").trim(),
          businessAddress: String(row["Business Address"] || row.businessAddress || row.BusinessAddress || "").trim(),
          city: String(row["City"] || row.city || row.CITY || "").trim(),
          state: String(row["State"] || row.state || row.STATE || "").trim(),
          pincode: String(row["Pincode"] || row.pincode || row.PINCODE || "").trim(),
          panNumber: String(row["PAN Number"] || row.panNumber || row.PANNumber || "").trim(),
          gstNumber: String(row["GST Number"] || row.gstNumber || row.GSTNumber || "").trim(),
          industryType: String(row["Industry Type"] || row.industryType || row.IndustryType || "").trim(),
          planType: String(row["Plan Type"] || row.planType || row.PlanType || "").trim(),
          existingAccountingSoftware: String(row["Existing Accounting Software"] || row.existingAccountingSoftware || row.ExistingAccountingSoftware || "").trim(),
          paymentMethod: String(row["Payment Method"] || row.paymentMethod || row.PaymentMethod || "").trim(),
        }));
...
export function validateTenantRegistrationRow(row: ImportTenantRegistrationRow, rowNumber: number): ValidationError[] {
  const errors: ValidationError[] = [];

  if (!row.businessName || row.businessName === "") {
    errors.push({
      row: rowNumber,
  ...
  if (row.planType && row.planType !== "") {
    const validPlanTypes = ["6_months_demo", "annual_subscription", "lifetime"];
    if (!validPlanTypes.includes(row.planType)) {
      errors.push({
        row: rowNumber,
        message: "Plan Type must be one of: 6_months_demo, annual_subscription, lifetime",
        field: "planType",
      });
    }
  }
  ...
  if (row.paymentMethod && row.paymentMethod !== "") {
    const validPaymentMethods = ["qr_code", "bank_transfer", "cash", "cheque", "online"];
    if (!validPaymentMethods.includes(row.paymentMethod)) {
      errors.push({
        row: rowNumber,
        message: "Payment Method must be one of: qr_code, bank_transfer, cash, cheque, online",
        field: "paymentMethod",
      });
    }
  }
  ...
  if (row.pincode && row.pincode !== "") {
    ...
    if (!pincodeRegex.test(row.pincode)) {
      errors.push({
        row: rowNumber,
  ...
  if (row.panNumber && row.panNumber !== "") {
    const panRegex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
    if (!panRegex.test(row.panNumber.toUpperCase())) {
      errors.push({
        row: rowNumber,
  ...
  if (row.gstNumber && row.gstNumber !== "") {
    ...
    if (!gstRegex.test(row.gstNumber.toUpperCase())) {
      errors.push({
        row: rowNumber,
