...
interface ProformaInvoicePrintDialogProps {
  open: boolean;
...
export function ProformaInvoicePrintDialog({ open, onOpenChange, invoice }: ProformaInvoicePrintDialogProps) {
  const { data: items = [], isLoading: itemsLoading } = useQuery<ProformaInvoiceItem[]>({
    queryKey: [`/api/proforma-invoices/${invoice?.id}/items`],
  ...
  const handleDownloadPDF = () => {
    ...
    const opt = {
      ...
      filename: `Proforma-Invoice-${invoice.invoiceNumber}.pdf`,
          ...
          {(itemsLoading || profileLoading) ? (
            ...
            <div className="relative">
              ...
              <div style={{ height: '4in' }} className="flex flex-col">
                ...
                {/* Header Row: Logo, Company Name, Invoice Title */}
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-sm font-semibold text-gray-700">#{invoice.invoiceNumber.split('-').pop()}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-4 mb-3">
                  ...
                  <div className="text-xs">
                    ...
                    <p className="font-semibold text-gray-800">{invoice.leadName}</p>
                    <p className="text-gray-600">{invoice.leadEmail}</p>
                    <p className="text-gray-600">{invoice.leadMobile}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-4 text-xs bg-gray-50 p-2 rounded">
                  <div>
                    ...
                    <span className="text-gray-600">{new Date(invoice.invoiceDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                  </div>
                  <div>
                    <span className="font-semibold text-gray-700">Due: </span>
                    <span className="text-gray-600">{new Date(invoice.dueDate).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                  </div>
                  <div>
              ...
              <div className="mb-4">
                <div className="flex justify-between items-center mb-2">
                  <div className="text-xs">
                    ...
                    <span className="text-gray-700">{numberToWords(Math.round(parseFloat(invoice.grandTotal)))}</span>
                  </div>
                  <div className="text-right text-xs space-y-1">
                    ...
                    <div className="flex justify-between gap-4">
                      ...
                      <span className="font-semibold">₹{parseFloat(invoice.totalTax).toFixed(2)}</span>
                    </div>
                    <div 
                    ...
                    >
                      <span>TOTAL:</span>
                      <span>₹{Math.round(parseFloat(invoice.grandTotal)).toLocaleString('en-IN')}</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mt-auto pt-4 border-t border-gray-300">
                ...
                <div>
                  ...
                  {invoice.termsAndConditions ? (
                    <div className="text-[10px] text-gray-600 whitespace-pre-wrap leading-relaxed">{invoice.termsAndConditions}</div>
                  ) : (
